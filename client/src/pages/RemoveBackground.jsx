  ...
  return (
   <div className='h-full overflow-y-scroll p-6 w-full flex items-start flex-wrap gap-4 text-slate-700'>
       ...
       <form onSubmit={handleSubmit} className='w-full max-w-lg p-4  bg-white rounded-lg border border-gray-200'>
          ...
          <input onChange={(e)=>setInput(e.target.files[0])} value={input} type='file'  accept='image/*' required className='w-full px-3 p-2 mt-2 outline-none text-sm rounded-md border border-gray-300 text-gray-600' />

         <p className='text-sm mt-2 text-gray-500'>Supports JPG, PNG and othser image formats</p>

          
            <button className='w-full flex justify-center items-center gap-2 bg-gradient-to-br from-[#F6AB41] to-[#FF4938] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
