...
function WriteArticle() {
  ...
  const articleLength = [
    {id:1,length:800, text:'Short ( 800 words )'},
    {id:2,length:1200, text:'Medium ( 800-1200 words )'},
    {id:3,length:1600, text:'Long ( 1200+ words )'},
  ]
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 w-full flex items-start flex-wrap gap-4 text-slate-700'>
       ...
       <form onSubmit={handleSubmit} className='w-full max-w-lg p-4  bg-white rounded-lg border border-gray-200'>
          ...
          <input onChange={(e)=>setInput(e.target.value)} value={input} type='text' placeholder='The futur of AI is ...' required className='w-full px-3 p-2 mt-2 outline-none text-sm rounded-md border border-gray-300' />

          <p className='mt-6 text-sm font-medium'>Article Length</p>
          <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
            {
              articleLength?.map((item,_)=>(
                <span onClick={()=>setSelectedlength(item)} key={item.id} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${item.length === selectedLength.length ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300'}`}>{item?.text}</span>
              ))
            }
          </div>
