...
import { Image, FileText, Eraser, UserCheck, Scissors, Type } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useUser, useClerk, PricingTable } from "@clerk/clerk-react";

const services = [
  { icon: <FileText size={32} className="text-pink-500" />, label: "Article Writer" },
  { icon: <Type size={32} className="text-yellow-400" />, label: "Blog Titles" },
  { icon: <Image size={32} className="text-green-400" />, label: "Image Generation" },
  { icon: <Scissors size={32} className="text-purple-500" />, label: "Background Removal" },
  { icon: <Eraser size={32} className="text-emerald-500" />, label: "Object Removal" },
    ...
    <section className="relative  w-full  md:pb-24 flex items-center justify-center text-center bg-black">
      <div className="relative max-w-6xl mx-auto px-4 py-16">
        ...
        <p className="text-lg md:text-2xl text-white/90 mb-12 font-medium drop-shadow-md">
          Your all-in-one AI platform for content creation, image generation, object removal, and more.
        </p>
